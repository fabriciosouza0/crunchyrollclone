...
export class DestaqueComponent implements OnInit {
  ...
  constructor(private tmdbApiService: TmdbApiService, private toastService: ToastService) { }

  ngOnInit(): void {
    const randIndex = this.rand(0, 2);
    this.mediaType = this.mediaTypes[randIndex];

    this.genres = this.tmdbApiService.genres(this.mediaType)
      .subscribe(genre => {
        const index = this.rand(0, genre.length);
        this.setGenre(genre[index].id);
      });

    this.highlight$ = this.tmdbApiService.discover(this.mediaType, { with_genres: this.genre })
      .pipe(
        delay(500),
        map((data) => {
          const index = this.rand(0, data.results.length);
